...
EXPERIMENT_STATES = [
    'waiting',
    'doing virtual reality drum radius 0.01',
    ## 'waiting',
...
class InterpolatingListener(object):
    def __init__(self,sockobj,dummy=True):
    ...
    def get_fly_xyz(self,prefer_obj_id=None):
        ...
        if self.dummy:
            ...
            timestamp = t
            theta = (2*pi*t) / 5.0
            x = eye_loc_default[0] + 0.04*np.cos( theta )
            y = eye_loc_default[1] + 0.04*np.sin( theta )
            z = eye_loc_default[2] + 0.1*np.sin(theta*.2)
            state_vecs = [ (x,y,z) ]
...
def main(connect_to_mainbrain=True,
    ...
    if UNIT_CUBE:
        ...
        faces['+z'] = [4,5,6,7]
        faces['-z'] = [0,1,2,3]
        for face_name, face_verts in faces.iteritems():
            face = TextureStimulus3D(texture = tree_texture,
                                     shrink_texture_ok=True,
                                     internal_format=gl.GL_RGBA,
    ...
    if DRUM:
        ...
        texture = Texture(filename)
        # cylinder
        drum = SpinningDrum( position=(.35,-.05,0), # center of WT
                             orientation=0.0,
                             texture=texture,
    ...
    if 0:
        ...
        corners_3d = [ # order: LL, UL, UR, LR
            (-0.61422014236450195, 0.13431060314178467, -0.018580993637442589),
            (0.57639992237091064, 0.1417778879404068, 0.02758500725030899),
            (0.56944286823272705, -0.1281694620847702, 0.043489802628755569),
            (-0.61817401647567749, -0.14428131282329559, -0.01322472095489502),
            ]
        corners_2d = [
            (1,401),
            (800,399),
    ...
    if 0:
        ...
        corners_3d = [
            # LL
            #(-0.63732963800430298, 0.14137864112854004, -0.010838760994374752),
            (-0.46700692176818848, 0.14311830699443817, -0.0052800653502345085),
            # UL
            (-0.4865739643573761, 0.14723047614097595, 0.23936140537261963),
            # UR
    ...
    while not quit_now:
        ...
        for wallname,wall in vr_walls.iteritems():
            ...
            if draw_stimuli:
                if DRUM and state_string.endswith('drum radius 0.01'):
                    drum.set(radius = 0.01)
